...
export function getLLMConfig(mode: 'ollama' | 'api' | 'deepseek-reasoner' = 'ollama'): LLMConfig {
  // If api key is set for specific providers, we might still want to respect the 'mode' argument
  // But original logic had environment variable overrides.
  // We will prioritize the 'mode' argument if provided, but fallback to environment checks if mode is default/undefined
  
  // Implementation for 'api' (Qwen)
  if (mode === 'api') {
      if (!process.env.QWEN_API_KEY) throw new Error("QWEN_API_KEY is missing");
      return {
          provider: 'custom',
          url: 'https://dashscope.aliyuncs.com/compatible-mode',
          chatModel: 'qwen-plus', // Using qwen-plus as requested
          embeddingModel: 'text-embedding-v1', // Not used for embedding in mixed mode
          stopWords: [],
          apiKey: process.env.QWEN_API_KEY,
      };
  }

  // Implementation for 'deepseek-reasoner' (DeepSeek)
  if (mode === 'deepseek-reasoner') {
      if (!process.env.DEEPSEEK_API_KEY) throw new Error("DEEPSEEK_API_KEY is missing");
      return {
          provider: 'custom',
          url: 'https://api.deepseek.com',
          chatModel: 'deepseek-chat', // User asked for "DeepSeek-V3.2 Thinking" which might apply to deepseek-reasoner or chat. Usually deepseek-chat is V3.
          embeddingModel: 'deepseek-embedding', // Not used
          stopWords: [],
  ...
  // ... (Together and Custom removed or kept as fallback? Keeping for backward compat if needed, but simplified for clarity)
  // Let's keep original fallback logic for Ollama
...
export async function chatCompletion(
  body: Omit<CreateChatCompletionRequest, 'model'> & {
    ...
    mode?: 'ollama' | 'api' | 'deepseek-reasoner';
  },
...
export async function chatCompletion(
  ...
  console.log('ChatCompletion Config:', { mode, url: config.url, model: body.model }); 
  
  // Remove 'mode' from body before sending to API to avoid errors from strict APIs
  const { mode: _, ...apiBody } = body;
  ...
  const {
    ...
    if (!result.ok) {
      ...
      console.error(`LLM Error (${result.status}):`, text);
      
      // Special handling for Ollama 404 (Model not found)
      if (result.status === 404 && config.provider === 'ollama') {
        await tryPullOllama(body.model!, text);
      }
      
      throw {
        retry: result.status === 429 || result.status >= 500,
        error: new Error(`LLM HTTP ${result.status}: ${text}`),
      };
    }
    ...
    if (body.stream) {
      ...
      const text = await result.text();
      try {
        const json = JSON.parse(text) as CreateChatCompletionResponse;
        const content = json.choices[0].message?.content;
        if (content === undefined) {
          throw new Error('Unexpected result from LLM: ' + text);
        }
        return content;
      } catch (e) {
        throw new Error(`Failed to parse LLM response JSON: ${e instanceof Error ? e.message : String(e)}. Response: ${text}`);
      }
    }
  });
