...
export class Conversation {
  ...
  constructor(serialized: SerializedConversation) {
    const { id, creator, created, createdGameTime, isTyping, lastMessage, numMessages, participants } = serialized;
  ...
  tick(game: Game, now: number) {
    ...
    // Determine dynamic threshold based on roles
    const role1 = game.playerDescriptions.get(player1.id)?.role;
    const role2 = game.playerDescriptions.get(player2.id)?.role;
    
    let threshold = CONVERSATION_DISTANCE;
    if (role1 === 'desk nurse' || role2 === 'desk nurse') {
      threshold = 4.5;
    } else if (role1 === 'inpatient nurse' || role2 === 'inpatient nurse') {
      threshold = 5.5;
    }
    ...
    if (member1.status.kind === 'walkingOver' && member2.status.kind === 'walkingOver') {
      if (playerDistance < threshold) {
        ...
        member1.status = { kind: 'participating', started: now, startedGameTime: game.world.time };
        member2.status = { kind: 'participating', started: now, startedGameTime: game.world.time };
  ...
  static start(game: Game, now: number, player: Player, invitee: Player) {
    ...
    game.world.conversations.set(
      ...
      new Conversation({
        id: conversationId,
        created: now,
        createdGameTime: game.world.time,
        creator: player.id,
        numMessages: 0,
        participants: [
          { playerId: player.id, invited: now, invitedGameTime: game.world.time, status: { kind: 'walkingOver' } },
          { playerId: invitee.id, invited: now, invitedGameTime: game.world.time, status: { kind: 'invited' } },
        ],
      }),
    );
  ...
  serialize(): SerializedConversation {
    const { id, creator, created, createdGameTime, isTyping, lastMessage, numMessages } = this;
    return {
      id,
      creator,
      created,
...
export const serializedConversation = {
  ...
  creator: playerId,
  created: v.number(),
  createdGameTime: v.optional(v.number()),
  isTyping: v.optional(
    v.object({
      playerId,
