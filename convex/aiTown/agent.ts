...
export class Agent {
  ...
  tick(game: Game, now: number) {
    ...
    // 1. Priority: Handle Invites IMMEDIATELY (Interrupting if needed)
    if (conversation && member && member.status.kind === 'invited') {
        ...
        const role = description?.role;
        let shouldAccept = false;

        switch (role) {
          case 'doctor':
          case 'emergency room doctor':
          case 'desk nurse':
          case 'nurse':
          case 'inpatient nurse':
          case 'diagnostic nurse':
          case 'student':
          case 'patient':
             shouldAccept = true;
             console.log(`Agent ${player.id} (${role}) PRIORITY accepting invite from ${otherPlayer.id}`);
             break;
          case 'admin':
             shouldAccept = !!otherPlayer.human || Math.random() < INVITE_ACCEPT_PROBABILITY;
             break;
          default:
             shouldAccept = !!otherPlayer.human || Math.random() < INVITE_ACCEPT_PROBABILITY;
             break;
        ...
        if (shouldAccept) {
            ...
            // INTERRUPT: Clear any in-progress operation
            if (this.inProgressOperation) {
                console.log(`Interrupting operation ${this.inProgressOperation.operationId} to accept invite.`);
    ...
    // 2. Secondary: Check In-Progress Operation
    if (this.inProgressOperation) {
    ...
    // conversation and member already defined above
    ...
    if (!conversation && !doingActivity && (!player.pathfinding || !recentlyAttemptedInvite)) {
      ...
      switch (role) {
        case 'admin':
          ...
          break;
        case 'patient':
          // Patient Logic: Active Workflow
          this.startOperation(game, now, 'patientDoSomething', {
        ...
        default:
          // Default behavior for undefined roles
          break;
      }
    ...
    if (conversation && member) {
      ...
      if (member.status.kind === 'walkingOver') {
        ...
        if (!player.pathfinding) {
          ...
          // Doctor Logic: Doctors stay put and wait for the patient to come to them.
          if (role === 'doctor' || role === 'Emergency room doctor' || role === 'nurse' || role === 'student' || role === 'Desk nurse') {
             return;
          }
          // Other roles can go here if checking logic is needed
          
          const otherDescription = game.playerDescriptions.get(otherPlayer.id);
          const targetIsDoctor = otherDescription?.role === 'doctor' || otherDescription?.role === 'Emergency room doctor';
          const targetIsNurse = otherDescription?.role === 'nurse';
          const targetIsStudent = otherDescription?.role === 'student';
          const targetIsDeskNurse = otherDescription?.role === 'Desk nurse';

          let destination;
          if (playerDistance < MIDPOINT_THRESHOLD || targetIsDoctor || targetIsNurse || targetIsStudent || targetIsDeskNurse) {
            destination = {
...
export async function runAgentOperation(ctx: MutationCtx, operation: string, args: any) {
  ...
  switch (operation) {
    ...
    case 'agentDoSomething':
      ...
      break;
    case 'patientDoSomething':
      reference = internal.aiTown.agentOperations.patientDoSomething;
      break;
    default:
...
export const agentSendMessage = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert('messages', {
      ...
      conversationPrompt: args.conversationPrompt,
    });
