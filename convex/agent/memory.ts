...
import { LLMMessage, chatCompletion, fetchEmbedding, EMBEDDING_DIMENSION } from '../util/llm';
...
export async function rememberConversation(
  ...
  const llmMessages: LLMMessage[] = [
    {
      role: 'user',
      content: `你是 ${player.name}，你刚刚结束了与 ${otherPlayer.name} 的对话。
      我希望你从 ${player.name} 的角度总结这次对话，使用第一人称代词如“我”，并加上你是否喜欢这次互动。`,
    },
  ];
  ...
  const importance = await calculateImportance(description, llmMode as 'ollama' | 'api' | 'deepseek-reasoner');
  let { embedding } = await fetchEmbedding(description);
  
  if (!embedding || embedding.length === 0) {
      console.warn('Embedding generation failed, utilizing zero-vector fallback.');
...
async function calculateImportance(description: string, mode: 'ollama' | 'api' | 'deepseek-reasoner') {
  const { content: importanceRaw } = await chatCompletion({
    messages: [
      {
        role: 'user',
        content: `在0到9的范围内，0代表纯粹的平凡（例如刷牙、整理床铺），9代表极度深刻（例如分手、收到大学录取通知书），请对以下记忆的深刻程度进行评分。
      记忆：${description}
      请在0到9的范围内回答。只回复数字，例如“5”`,
      },
    ],
    temperature: 0.0,
    max_tokens: 1,
...
async function reflectOnMemories(
  ...
  // should only reflect if lastest 100 items have importance score of >10 (lowered from 500)
  const sumOfImportanceScore = memories
    .filter((m) => m._creationTime > (lastReflectionTs ?? 0))
    .reduce((acc, curr) => acc + curr.importance, 0);
  const shouldReflect = sumOfImportanceScore > 10;

  if (!shouldReflect) {
    console.debug('sum of importance score = ', sumOfImportanceScore, 'threshold = 10. Not reflecting.');
    return false;
  }
  console.debug('sum of importance score = ', sumOfImportanceScore, 'Reflecting...');
  const prompt = ['[no prose]', '[Output only JSON]', `你是 ${name}，关于你的陈述: `];
  memories.forEach((m, idx) => {
    prompt.push(`陈述 ${idx}: ${m.description}`);
  });
  prompt.push('你能从以上陈述中推断出哪3个高层次的见解？');
  prompt.push(
    '以JSON格式返回，其中key是促成你看法的输入陈述列表，value是你的看法。使响应可由Typescript JSON.parse()函数解析。切勿在响应中转义字符或包含“\\n”或空格。',
  );
  prompt.push(
  ...
  try {
    ...
    const memoriesToSave = await asyncMap(insights, async (item) => {
      ...
      const importance = await calculateImportance(item.insight, llmMode as 'ollama' | 'api' | 'deepseek-reasoner');
      let { embedding } = await fetchEmbedding(item.insight);
      if (!embedding || embedding.length === 0) {
          console.warn('Embedding generation failed (reflection), utilizing zero-vector fallback.');
          embedding = new Array(EMBEDDING_DIMENSION).fill(0);
      }
